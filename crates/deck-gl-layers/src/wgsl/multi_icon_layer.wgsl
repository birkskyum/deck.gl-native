...
//
// `@deck.gl/layers` multi-icon-layer.wgsl without the collision filter: the characters of a
// TextLayer as icons from a font atlas, with optional SDF rendering and outlines.
...
fn fragmentMain(inp: Varyings) -> @location(0) vec4<f32> {
  ...
  let a = alpha * color.a * layer.opacity;
