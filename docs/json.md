...
| `SimpleMeshLayer` | `mesh` (an OBJ file path or URL, or an object with `positions`, `normals`, `colors`, `texCoords`, `indices`), `texture`, `sizeScale`, `wireframe`, `_instanced`, `getPosition`, `getColor`, `getOrientation`, `getScale`, `getTranslation`, `getTransformMatrix` |
| `Tile3DLayer` | `data` (the URL of a `tileset.json`), `maximumScreenSpaceError`, `color`, `_lighting`. `b3dm` and glTF tiles are drawn; point cloud (`pnts`) and composite (`cmpt`) tiles are not read yet |
| `A5Layer` | `getPentagon` (an A5 index, hexadecimal or decimal), plus every `PolygonLayer` prop |
| `H3ClusterLayer` | `getHexagons` (a list of H3 indexes per row, merged into the outline of the area they cover), plus every `PolygonLayer` prop |
| `TerrainLayer` | `elevationData` (a `{z}/{x}/{y}` template for tiled terrain, or one image URL with `bounds`), `texture`, `elevationDecoder` (`mapbox`, `terrarium`, or an object with `rScaler`, `gScaler`, `bScaler`, `offset`), `meshMaxError`, `bounds`, `color`, `wireframe`, `minZoom`, `maxZoom`, `tileSize`, `maxRequests` |
| `ScenegraphLayer` | `scenegraph` (a `.glb` or `.gltf` path or URL; external buffers and images resolve relative to it), `sizeScale`, `sizeMinPixels`, `sizeMaxPixels`, `_lighting` (`flat` or `pbr`), `getPosition`, `getColor`, `getOrientation`, `getScale`, `getTranslation`, `getTransformMatrix` |
