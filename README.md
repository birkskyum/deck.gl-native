...
- `ScatterplotLayer`, `LineLayer`, `SolidPolygonLayer` (filled, extruded, wireframe, holes), `PathLayer` (joints, caps, billboard), `ArcLayer`, `BitmapLayer`, `IconLayer`, `TextLayer` (font atlas from any TrueType font, SDF outlines, backgrounds, wrapping), `ColumnLayer`, `GridCellLayer`, `PointCloudLayer`, the composite `PolygonLayer` and `GeoJsonLayer`, the aggregation layers `HexagonLayer`, `GridLayer` and `ScreenGridLayer` (CPU binning, sum/mean/min/max/count, quantize/linear/quantile/ordinal scales, percentile cutoffs) and `HeatmapLayer` (GPU weights texture with additive and max blending, colour ramp texture), `ContourLayer` (marching squares isolines and isobands over the grid aggregation), and `TileLayer` (deck.gl's tileset: frustum culled OSM quadtree, best available refinement, cache, background loading on a thread pool; raster tiles from URL templates or any loader and sub layer renderer), and `TripsLayer` and `GreatCircleLayer` from `@deck.gl/geo-layers`
